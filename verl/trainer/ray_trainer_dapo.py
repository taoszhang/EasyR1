...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            if "multi_modal_data" in test_batch.non_tensor_batch.keys() and "ret_img" in test_batch.non_tensor_batch.keys():
                test_gen_batch = test_batch.pop(
                    ...
                    non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "entity", "ret_img", "ret_img_score"],
                )
            elif "multi_modal_data" in test_batch.non_tensor_batch.keys():
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "entity"],
                )
            else:
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", "entity"],
                )
            ...
            # Store ground truths and answers
        ...
        self._maybe_log_val_generations(sample_inputs, sample_outputs, sample_scores, sample_ground_truths, sample_answers)
    ...
    def _make_batch_data(self, metrics: Dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            meta_info = {
                ...
                'repetition_penalty': self.config.worker.rollout.repetition_penalty,
            ...
            if "multi_modal_data" in new_batch.non_tensor_batch.keys() and "ret_img" in new_batch.non_tensor_batch.keys():
                gen_batch = new_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data", "entity", "ret_img", "ret_img_score"],
                    meta_info_keys=["n", "temperature", "top_p", "repetition_penalty", "min_pixels", "max_pixels", "video_fps"],
                )
            ...
            if self.config.algorithm.online_filtering:
                ...
                # 所以要求就是一个sample里面的样本，既不能比reward最小值低，也不能比最大值高
            ...
            if current_batch_size < rollout_batch_size:
                ...
                if max_try_make_batch <= 0 or num_try_make_batch < max_try_make_batch:
                    ...
                    # print(f"[Warning] {num_try_make_batch=} >= {max_try_make_batch=}. Returning current batch with {current_batch_size} samples.")
                    # if self.config.algorithm.online_filtering:
