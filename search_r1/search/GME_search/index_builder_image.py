...
class FeatureExtractor:
    def __init__(self, args):
        ...
        # self.model.eval()
        self.gme_model = GmeQwen2VL(args.model_path)
    
        self.dataset = ImageDataset(args.input_file)
    ...
    def extract_features(self):
        ...
        with torch.no_grad():
            for titles, images in tqdm(self.data_loader, desc="Extracting Features", leave=True):
                input_texts = [f"This image shows: {title}" for title in titles]
                fused_embeddings = self.gme_model.get_fused_embeddings(texts=input_texts, images=images)
                all_features.append(fused_embeddings.cpu())
                all_titles.extend(titles)  # Collect titles corresponding to the images
                    
        all_features = torch.cat(all_features, dim=0).numpy()
        self.feature_dim = all_features.shape[-1]  # Update feature dimension
    ...
    def build_index(self):
        ...
        features = np.memmap(features_file_path, dtype=np.float32, mode='r', shape=(len(self.dataset), self.feature_dim))
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default='GME', help='Name of the CLIP model used')
    parser.add_argument('--model_path', type=str, default='/data/tzhang/model/gme-Qwen2-VL-7B-Instruct', help='Path to the CLIP model')
    ...
    parser.add_argument('--save_dir', type=str, default='/data/tzhang/project/Infoseek_multi_hop/search_engine/GME_7B_text_new', help='Root directory to save features')
    ...
    parser.add_argument('--feature_dim', type=int, default=1536, help='Feature dimension size (e.g., 768 for ViT-L/14)')
