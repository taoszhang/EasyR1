...
class LLMGenerationManager:
    def __init__(
        self,
        tokenizer,
        actor_rollout_wg, # 现在dapo里面改成了名为actor_rollout_ref_wg
    ...
    def _postprocess_responses(self, responses: torch.Tensor) -> torch.Tensor:
        ...
        responses_str = [truncate_response(resp) for resp in responses_str]
    ...
    def run_llm_loop(self, gen_batch, initial_input_ids: torch.Tensor) -> Tuple[Dict, Dict]:
        ...
        self.entity = gen_batch.non_tensor_batch.pop('entity')
        self.entity = np.repeat(self.entity, repeats=self.config.rollout_n, axis=0)
    ...
    def execute_predictions(self, predictions: List[str], pad_token: str, active_mask=None, do_search=True) -> List[str]:
        ...
        text_search_queries = [content for action, content in zip(cur_actions, contents) if action == 'text_search']
        if do_search:
            text_search_results = self.batch_search(text_search_queries)
            assert len(text_search_results) == sum([1 for action in cur_actions if action == 'text_search'])
        else:
            text_search_results = [''] * sum([1 for action in cur_actions if action == 'text_search'])
        ...
        for i, (action, active) in enumerate(zip(cur_actions, active_mask)):
            ...
            if not active:
                ...
                if action == 'answer':
                    ...
                    next_obs.append(f'\n\n<information>{text_search_results.pop(0).strip()}</information>\n\n')
                    dones.append(0)
                    valid_action.append(1)
                    is_search.append(1)
                    ...
                    next_obs.append(self.image_search(i, contents[i]))
                    dones.append(0)
                    valid_action.append(1)
                    is_search.append(1)
                else:
                    next_obs.append(f'\nMy previous action is invalid. \
If I want to search, I should put the query between <text_search> and </text_search>. \
If I want to give the final answer, I should put the answer between <answer> and </answer>. Let me try again.\n')
#                     next_obs.append(f'\n\nMy previous action is invalid. \
# If I want to search for image information, I should generate <image_search> image n </image_search>. \
# If I want to search for other types of information, I should place the query between <text_search> and </text_search>. \
# If I want to give the final answer, I should put the answer between <answer> and </answer>. Let me try again.\n')
                    dones.append(0)
                    valid_action.append(0)
                    is_search.append(0)
            
        assert len(text_search_results) == 0
    ...
    def image_search(self, index, query):
#         if self.ret_img:
#             if query.lower().strip() == 'image 1':
#                 return f'\n\n<information>This image shows {self.ret_img[index][0]}</information>\n\n'
#             elif query.lower().strip() == 'image 2':
#                 return f'\n\n<information>This image shows {self.ret_img[index][1]}</information>\n\n'
#             else:
#                 return f'\n\nMy previous action is invalid. \
# I need to generate a query based on the image index. \
# For example, to view the first image, I should generate <image_search> image 1 </image_search>. \
# Let me try again.\n'
#         else:
        if query.lower().strip() == 'image 1':
            return f'\n\n<information>This image shows {self.entity[index][0]}</information>\n\n'
        elif query.lower().strip() == 'image 2':
            # return f'\n\n<information>This image shows {self.entity[index][1]}</information>\n\n'
            if len(self.entity[index]) > 1:
                return f'\n\n<information>This image shows {self.entity[index][1]}</information>\n\n'
            else:
                return f'\n\n<information>The image 2 does not exist</information>\n\n'
        else:
            return f'\n\nMy previous action is invalid. \
I need to generate a query based on the image index. \
For example, to view the first image, I should generate <image_search> image 1 </image_search>. \
