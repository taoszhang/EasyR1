...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        output_path = os.path.join('/data/tzhang/project/EasyR1/datasets/infoseek_bridge', f'part_{i}.parquet')
        if i < 8:
            save_path = os.path.join('/data/tzhang/project/EasyR1/datasets', f'infoseek_bridge_train', f'train-{i:05d}-of-00010.parquet')
        else:
            save_path = os.path.join('/data/tzhang/project/EasyR1/datasets', f'infoseek_bridge_test', f'test-{i:05d}-of-00010.parquet')
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        df.to_parquet(save_path, index=True)
        print(f"Saved part {i} to {save_path}")

    # train_data = processed_data[:len(processed_data)*7 // 10]
    # test_data = processed_data[len(processed_data)*7 // 10:]

    # train_df = pd.DataFrame(train_data)
    # test_df = pd.DataFrame(test_data)

    # result_folder = '/data/tzhang/project/EasyR1/datasets/infoseek_bridge'
    # os.makedirs(result_folder, exist_ok=True)
    # # import pdb; pdb.set_trace()
    # train_output_path = os.path.join(result_folder, 'train.parquet')
    # test_output_path = os.path.join(result_folder, 'test.parquet')
